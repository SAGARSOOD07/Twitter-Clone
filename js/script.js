let audience_btn = document.querySelector('.audience_btn');
let show_bx_1 = document.querySelector('.show_bx_1');
...
Array.from(document.getElementsByClassName('check_audience')).forEach((i, a) => {
    i.addEventListener('click', () => {
        ...
        if (a === 0) {
            audience_btn.innerHTML = 'Everyone <i class="fas fa-angle-down"></i>';
            audience_btn.style.color = "#1ca1f1";
            audience_btn.style.borderColor = "#1ca1f1";
        } else {
            audience_btn.innerHTML = 'Twitter Circle <i class="fas fa-angle-down"></i>';
...
tweet_text_area.addEventListener('keyup', () => {
    ...
    let b = parseInt((a / 2)); //value that are coming in point will be removed 
    let c = (100 - b);
